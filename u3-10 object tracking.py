...
# captures the first frame from the video stream.
#  ret is a boolean indicating if the frame was read successfully,
#  and frame is the captured image.
ret, frame = cap.read()
#opens a window named 'select' where the user can manually
#  select a Region of Interest (ROI) on the first frame.
#  The ROI is returned as bbox which contains the coordinates (x, y, w, h).
